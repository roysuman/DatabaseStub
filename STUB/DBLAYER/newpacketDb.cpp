...
bool createPacketDb::GenarateRawPacket::insertGeneratedPacket ( actSeriesStruct *& conversationPacket ){
    std::cout<<"createPacketDb::GenarateRawPacket::insertGeneratedPacket\n";
    ...
    try{
		...
		std::cerr<<"cgt exception ="<<ba.what()<<std::endl;
                exit(0);
	}
	//initialize the conversation No and packet no
        conPacketNo * conversationPacketNo = clasInstance->getAssignedConPackNo();
        conversationPacket->actRawData [ 0 ].packetNumber = conversationPacketNo->packetNo + 1 ;
        conversationPacket->converationId = conversationPacketNo->conversationNo + 1;
         clasInstance->insertGeneratedPacket ( conversationPacket ) ;




        //now update the query string
                //added on 3_6_2014
...
//                    }
                    ...
                    std::cout<<"current packet ="<<currentPacket<<std::endl;
                    ...
                    std::cout<<"NOW DISSECT QUERY \n";


                  //  Mysql::getQueryOnly ( packetSettingDataArray , queryDataArray );
                    actRawDataStruct *newRawDataArray = new actRawDataStruct  [ 2  ];
...
bool createPacketDb::GenarateRawPacket::deletePacketRegardingConversationId( int conversatinId ){
	...
	try{
                ...
                returnValue =instance->deletePacketRegardingConversationId( conversatinId) ; //instance.deletePacketRegardingConversationId ( conversationId );
                std::cerr<<"return value="<<returnValue<<std::endl;
		delete instance;

	}
	catch ( std::bad_alloc &ba ){
                FILEH<<"cgt exception at createPacketDb::GenarateRawPacket::deletePacketRegardingConversationId = "<<ba.what()<<std::endl;
		exit(0);
	}
