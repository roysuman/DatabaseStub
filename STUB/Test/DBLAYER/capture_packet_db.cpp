...
STUB_INT lowerLimitConversationDiary = 0;
 ...
 * =====================================================================================
 */
bool captureDb::StoreRawPacket::copyPacket ( bool cliServer , 
		conversation *currentConversation , 
		rawDataStruct *rawData ){ //true if app
	std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__<<"] Line ["
		<<__LINE__<<"] DateTime ["<<__DATE__<<"_"<<__TIME__<<"]\n";
	if ( currentConversationId < 0 ) return false;
	std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__<<"] Line ["
		<<__LINE__<<"] DateTime ["<<__DATE__<<"_"<<__TIME__<<"] [CLIENT array\
	       	index]>"<<currentConversation->currentPositionClientArray<<
	...
	try{
		if ( cliServer ){ // this is a client packet
			std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__
				...
				<<__TIME__<<"]"<<__FUNCTION__<<"client packet\n";
			currentConversation->allPreviousPacketClient 
				[(++ currentConversation->currentPositionClientArray ) ].
				prevSequenceNumber = rawData->sequenceNumber;
			currentConversation->allPreviousPacketClient 
				[currentConversation->currentPositionClientArray ].
				prevAcknowledgeNumber = rawData->acknowledgeNumber;
			currentConversation->allPreviousPacketClient 
				[ currentConversation->currentPositionClientArray ].
				prevLength = rawData->length;
			currentConversation->allPreviousPacketClient 
				[ currentConversation->currentPositionClientArray ].
				tcpFlag.ACK = rawData->tcpFlag.ACK;
			
		}
		else {
			std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__
				<<"] Line ["<<__LINE__<<"] DateTime ["<<__DATE__<<"_"
				<<__TIME__<<"] server packet\n";
			currentConversation->allPreviousPacketServer 
				[ ( ++currentConversation->currentPositionServerArray  ) ].
				prevSequenceNumber = rawData->sequenceNumber;
			currentConversation->allPreviousPacketServer 
				[ currentConversation->currentPositionServerArray ].
				prevAcknowledgeNumber = rawData->acknowledgeNumber;
 ...
 * =====================================================================================
 */
bool captureDb::StoreRawPacket::searchMother( bool cliServer  , 
		conversation  *currentConversation , 
	...
	if ( currentConversationId < 0 ) return false;
	if ( cliServer ){
		for ( STUB_INT i = currentConversation->currentPositionServerArray ;
			       	i>=0 ; i -- ){
			if ( ( rawData->acknowledgeNumber == 
						currentConversation->allPreviousPacketServer [ i ].prevAcknowledgeNumber ) && 
					( rawData->sequenceNumber ==  
					  ( currentConversation->allPreviousPacketServer [ i ].prevSequenceNumber 
					    + currentConversation->allPreviousPacketServer [ i ].prevLength ) ))
				return true;
		}
		return false;
	} else {
		for ( STUB_INT i = currentConversation->currentPositionClientArray ;
			       	i>=0 ; i -- ){
			if ( ( rawData->acknowledgeNumber == 
						currentConversation->allPreviousPacketClient [ i ].prevAcknowledgeNumber )&& 
					( rawData->sequenceNumber == 
					  currentConversation->allPreviousPacketClient [ i ].prevSequenceNumber +
					  currentConversation->allPreviousPacketClient [ i ].prevLength  ) )
				return true;
 ...
 * =====================================================================================
 */
bool captureDb::StoreRawPacket::checkSimultaneouslyNewPacket ( bool cliServer ,
	       	conversation *currentConversation , rawDataStruct *rawData ){
	std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__<<"] Line ["
		<<__LINE__<<"] DateTime ["<<__DATE__<<"_"<<__TIME__<<"]\n";
	if ( currentConversationId < 0 ) return false;
	std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__<<"] Line ["
		<<__LINE__<<"] DateTime ["<<__DATE__<<"_"<<__TIME__
		<<"] Current position CLIENT  array\t"
		<<currentConversation->currentPositionClientArray
		<<"currnet position SERVER array\t"
		<<currentConversation->currentPositionServerArray<<'\n';
	...
	if ( cliServer ){ //client packet
		for ( STUB_INT i = currentConversation->currentPositionClientArray ;
			       	i>= 0 ; -- i ){
			if ( ( rawData->sequenceNumber ==( currentConversation->
							allPreviousPacketClient [ i ].prevSequenceNumber + 
							currentConversation->
							allPreviousPacketClient [ i ].prevLength ) ) && 
 ...
 * =====================================================================================
 */
bool captureDb::StoreRawPacket::validPack ( bool cliServer , 
		conversation *currentConversation , rawDataStruct *rawData){
	std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__<<"] Line ["
		<<__LINE__<<"] DateTime ["<<__DATE__<<"_"<<__TIME__<<"]\n";
	if ( currentConversationId < 0 ) return false;
	std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__<<"] Line ["
		<<__LINE__<<"] DateTime ["<<__DATE__<<"_"<<__TIME__
		<<"] [CLIENT  array index]:"<<currentConversation->currentPositionClientArray
		<<"[SERVER array index]"<<currentConversation->currentPositionServerArray<<'\n';
 ...
 * =====================================================================================
 */
bool captureDb::StoreRawPacket::checkDbGreeting ( conversation *currentConversation , 
		rawDataStruct *rawData ){
	std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__<<"] Line ["
		<<__LINE__<<"] DateTime ["<<__DATE__<<"_"<<__TIME__<<"]\n";
	if ( currentConversationId == -1 ) return true;
		...
		std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__
			<<"] Line ["<<__LINE__<<"] DateTime ["<<__DATE__<<"_"
			<<__TIME__<<"checkDBGreeting "<<currentConversationId<<std::endl;
		return false ;
	...
	for ( STUB_INT i = currentConversation->currentPositionClientArray ; 
			i>= 0 ; -- i ){
		if ( ( currentConversation->allPreviousPacketClient [ i ].prevLength == 0 )
			       	&& ( currentConversation->allPreviousPacketClient [ i ].tcpFlag.ACK )
			       	&& ( rawData->acknowledgeNumber == 
					currentConversation->allPreviousPacketClient [ i ].prevSequenceNumber
				       	+ currentConversation->allPreviousPacketClient [ i ].prevLength )
			       	&& ( rawData->sequenceNumber == 
...
bool captureDb::StoreRawPacket::packetHandler ( rawDataStruct *rawData ){
    ...
    if ( rawData->sourcePort != 3306 ){
        std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__
		<<"] Line ["<<__LINE__<<"] DateTime ["<<__DATE__<<"_"<<__TIME__
		<<"]PACKET COMING FROM CLIENT(APPLICATION)\n";
	 ...
	 *-----------------------------------------------------------------------------*/
        if ( ( rawData->length == 0 )  && rawData->tcpFlag.ACK ){
            if ( synFin ){// if previous packet was connection setup or terminated packet
                std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__
			<<"] Line ["<<__LINE__<<"] DateTime ["<<__DATE__<<"_"
			<<__TIME__<<"]prev packet was connection termineted packet\n";
                synFin = false;
                return true;
	     ...
	     *-----------------------------------------------------------------------------*/
            ...
            if ( validPack ( false , &conversationDiary [ currentConversationId ] ,
				    rawData ) ){
                std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__
			<<"] Line ["<<__LINE__<<"] DateTime ["<<__DATE__<<"_"
			<<__TIME__
			<<"] it's a valid acknowledgement packet of this conversation\n";
                //store tis packet into packet with respect to current conversationId
                queryHandlerInstance.insertPacket( true , currentConversationId ,
			       	++packetNumber , rawData);
                copyPacket ( true ,&conversationDiary [ currentConversationId ] ,
			       	rawData );
                return true;
            }
        }
	 ...
	 *-----------------------------------------------------------------------------*/
        if ( (validPack ( false , &conversationDiary [ currentConversationId ] ,
				       	rawData )) || 
			checkSimultaneouslyNewPacket( true , 
				&conversationDiary [ currentConversationId] ,
			       	rawData ) ){// !
		 ...
		 *
		 *-----------------------------------------------------------------------------*/
		if ( queryHandlerInstance.insertPacketSetting ( true , 
					currentConversationId , ++packetNumber , 
					rawData ) ){
		    if ( queryHandlerInstance.insertPacket ( true , 
					    currentConversationId , packetNumber , 
					    rawData) )
			    return true ;
		    else return false;
        ...
        if ( searchMother ( false ,&conversationDiary [ currentConversationId ] ,
			       	rawData ) ){
		copyPacket ( true , &conversationDiary [ currentConversationId ] , 
				rawData);
		std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__
			<<"] Line ["<<__LINE__<<"] DateTime ["<<__DATE__
			<<"_"<<__TIME__<<"] Middle packet \n";
		if ( queryHandlerInstance.insertPacket ( true , currentConversationId
				       	, ++packetNumber , rawData) ){
			std::cout<<" File Name ["<<__FILE__<<" ] Function [ "
				<<__FUNCTION__<<"] Line ["<<__LINE__<<"] DateTime ["
				<<__DATE__<<"_"<<__TIME__<<"] middle packet inserted\n";
			return true ;
        ...
        //but if db handshaking occurs 1st then it's being captured
         ...
         copyPacket ( true , &conversationDiary [ ++currentConversationId ] ,
			 rawData );
        if ( queryHandlerInstance.insertPacketSetting ( true , 
				currentConversationId , ++packetNumber ,rawData) ){
            std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__
		    <<"] Line ["<<__LINE__<<"] DateTime ["<<__DATE__<<"_"
		    <<__TIME__<<"] now insert packet\n";
            queryHandlerInstance.insertPacket ( true , currentConversationId ,
			    packetNumber , rawData );
                    return true;
                }
     ...
     *-----------------------------------------------------------------------------*/
	    ...
	    std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__
		    <<"] Line ["<<__LINE__<<"] DateTime ["<<__DATE__<<"_"
		    <<__TIME__<<"]"<<__FUNCTION__<<"it is a SERVER packet\n";
	    if ( ( rawData->length == 0 )  && rawData->tcpFlag.ACK ){
		    if ( synFin ){// if previous packet was connection setup or terminated packet
			    std::cout<<" File Name ["<<__FILE__<<" ] Function [ "
				    <<__FUNCTION__<<"] Line ["<<__LINE__<<"] Date\
				    Time ["<<__DATE__<<"_"<<__TIME__
				    <<"]previous packet was connection termi\
				    neted packet or ack\n";
			    return true;
		     ...
		     *-----------------------------------------------------------------------------*/
		    if ( validPack ( true , &conversationDiary [ currentConversationId ]
					    , rawData) ){
			    copyPacket ( false , 
					    &conversationDiary [ currentConversationId ] ,
					    rawData );
			    std::cout<<" File Name ["<<__FILE__<<" ] Function [ "
				    <<__FUNCTION__<<"] Line ["<<__LINE__<<"] Date\
				    Time ["<<__DATE__<<"_"<<__TIME__
				    <<"]it's a valid acknowledgement packet of this\
				    conversation\n";
			    //store tis packet into packet with respect to current conversationId
			    if (queryHandlerInstance.insertPacket ( false , 
						    currentConversationId , ++packetNumber
						    , rawData ) ){
				    std::cout<<" File Name ["<<__FILE__<<" ] Function [ "
					    <<__FUNCTION__<<"] Line ["<<__LINE__<<"] Date\
	    ...
	    if ( validPack ( true , &conversationDiary [ currentConversationId ] , 
				    rawData) ){ // !
		    if ( checkDbGreeting ( &conversationDiary [ currentConversationId ]
					    , rawData ) ){
			    std::cout<<" File Name ["<<__FILE__<<" ] Function [ "
				    <<__FUNCTION__<<"] Line ["<<__LINE__<<"] Date\
				    Time ["<<__DATE__<<"_"<<__TIME__
				    <<"]This is a greetings packet from db\n";
			    if ( queryHandlerInstance.insertDbGreeting  
					    ( ++currentConversationId , ++packetNumber ,
					      rawData) ){
				    queryHandlerInstance.insertPacket  ( false ,
	    ...
	    if ( searchMother ( false , &conversationDiary [ currentConversationId ] ,
		    ...
		    std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__
			    <<"] Line ["<<__LINE__<<"] DateTime ["<<__DATE__
			    <<"_"<<__TIME__<<"]Middle packet \n";
		    if ( queryHandlerInstance.insertPacket ( false , currentConversationId
					    , ++ packetNumber , rawData ) ){
			    std::cout<<" File Name ["<<__FILE__<<" ] Function [ "
				    <<__FUNCTION__<<"] Line ["<<__LINE__<<"] DateTime ["
				    <<__DATE__<<"_"<<__TIME__<<"]Reply packet inserted \n";
			    return true;
		    }
		    return false;
	    }else {
		    ...
		    std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__
			    <<"] Line ["<<__LINE__<<"] DateTime ["<<__DATE__<<"_"
			    <<__TIME__<<"]GREETING packet from SERVER \n";
		    /*-----------------------------------------------------------------------------
...
/* 		    if( currentConversationId >= 0 ){
		    ...
		    */
		    if ( queryHandlerInstance.insertDbGreeting  ( ++currentConversationId ,
					    ++packetNumber , rawData) ){
			    std::cout<<" File Name ["<<__FILE__<<" ] Function [ "
				    <<__FUNCTION__<<"] Line ["<<__LINE__<<"] DateTime ["
				    <<__DATE__<<"_"<<__TIME__<<"]conversation id ="
				    <<currentConversationId<<std::endl;
			    std::cout<<" File Name ["<<__FILE__<<" ] Function [ "
				    <<__FUNCTION__<<"] Line ["<<__LINE__<<"] DateTime ["
				    <<__DATE__<<"_"<<__TIME__<<"]"
				    <<conversationDiary [ 0].conversationId<<"check\n";
			    std::cout<<" File Name ["<<__FILE__<<" ] Function [ "
				    <<__FUNCTION__<<"] Line ["<<__LINE__<<"] DateTime ["
				    <<__DATE__<<"_"<<__TIME__<<"]"
				    <<conversationDiary->currentPositionServerArray<<std::endl;
			    std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__
				    <<"] Line ["<<__LINE__<<"] DateTime ["<<__DATE__<<"_"
				    <<__TIME__<<"]conversationDiary->currentPositionServerArray"
				    <<conversationDiary->currentPositionServerArray<<std::endl;
			//    conversationDiary->allPreviousPacketServer [  ( conversationDiary->currentPositionServerArray ) ].prevSequenceNumber = rawData->sequenceNumber;
			    ...
			    captureDb::StoreRawPacket::copyPacket ( false , 
					    &conversationDiary [ currentConversationId ] , rawData );
			    std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__
				    <<"] Line ["<<__LINE__<<"] DateTime ["<<__DATE__<<"_"<<__TIME__
				    <<"]"<<__FUNCTION__<<"app array\t"
				    <<conversationDiary [currentConversationId].currentPositionClientArray
				    <<"db arraarrau**************** \t"
				    <<conversationDiary [currentConversationId].currentPositionServerArray
				    <<"conversation Id\t"<<currentConversationId<<std::endl;
			    if ( queryHandlerInstance.insertPacketSetting ( false , 
						    currentConversationId , 
						    packetNumber ,rawData) ){
				    queryHandlerInstance.insertPacket ( false , currentConversationId ,
						    packetNumber , rawData );
				    return true;
			    }else{
				    std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__
					    <<"] Line ["<<__LINE__<<"] DateTime ["<<__DATE__<<"_"
					    <<__TIME__<<"] Cant insert packet\n";
				    return false;
			    }
			    return true;
		    }else {
			    std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__
				    <<"] Line ["<<__LINE__<<"] DateTime ["<<__DATE__<<"_"
				    <<__TIME__<<"]cant insert into greeting DB\n";
			    return false;
		    }
	    }
    }
...
bool captureDb::StoreRawPacket::updateQueryTypeQueryAtDatabase ( ){
	...
	try {
		...
		std::cerr<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__
			<<"] Line ["<<__LINE__<<"] DateTime ["<<__DATE__<<"_"
			<<__TIME__<<"]exception cought "<<ba.what()<<std::endl;
		exit ( -1 );
	}
        ...
        try{
            ...
            std::cerr<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__
		    <<"] Line ["<<__LINE__<<"] DateTime ["<<__DATE__<<"_"
		    <<__TIME__<<"]exception caught "<<ba.what()<<std::endl;
	    exit ( -1 );
        }
	for ( STUB_INT i = 0 ; i < totalPacket ;++i ){
		try{
			boost::any_cast< STUB_INT ( * )( actRawDataStruct * , queryData *& ,
				       	STUB_INT , STUB_INT )> ( boost::get < boost::any > 
						( proto::Proto::initDissect
						  ( packetSettingDataArray [ i].protocolName , 1 
						    ) ) )  
						( packetSettingDataArray ,  queryDataArray , i , 0 );
		}
	...
	for ( STUB_INT i = 0 ; i<totalPacket ; ++i){
		std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__<<"] Line ["
			<<__LINE__<<"] DateTime ["<<__DATE__<<"_"<<__TIME__
			<<"] Query:"<<queryDataArray[i].query<<std::endl;
		std::cout<<" File Name ["<<__FILE__<<" ] Function [ "<<__FUNCTION__<<"] Line ["
			<<__LINE__<<"] DateTime ["<<__DATE__<<"_"<<__TIME__<<"] Query Type:"
